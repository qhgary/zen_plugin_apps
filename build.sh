...
obfuscate_replay_html() {
    ...
    local crate_version
    crate_version="$(grep -m1 '^version' "$ROOT_DIR/Cargo.toml" | cut -d '"' -f2)"
    local obf_seed
    obf_seed="$(( $(echo "$crate_version" | cut -d. -f1) * 1048576 + $(echo "$crate_version" | cut -d. -f2) * 1024 + $(echo "$crate_version" | cut -d. -f3) ))"
