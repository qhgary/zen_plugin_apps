...
obfuscate_replay_html() {
    ...
    # Obfuscation seed: always random per build (mirrors applications/build.bat).
    # Version-derived determinism was removed on purpose: byte-reproducible
    # artifacts let anyone rebuild a matching reference and diff it against
    # published output. Incremental builds are unaffected -- the source-content
    # snapshot gate, not artifact bytes, decides rebuilds.
    local obf_seed
