...
function JSDialogTooltip()
{
    ...
    this.OnClickColseButton=function(e)
    {
        ...
        if (this.HQChart && this.HQChart.ChartCorssCursor)
        {
            ...
            // 标记为用户手动关闭，下次触摸时恢复十字光标
            this.HQChart.UserClosedCorssCursor=true;
            this.HQChart.Draw();
    ...
    this.Create=function()
    {
        ...
        divTitle.ontouchstart=(e)=>{ if(e.touches.length>0){ this.OnMouseDownTitle(e.touches[0]); } }
        ...
        divClose.ontouchstart=(e)=>{ this.OnClickColseButton(e); e.preventDefault(); }
    ...
    this.OnMouseDownTitle=function(e)
    {
        ...
        // 触摸事件支持（移动端拖拽小窗）
        document.ontouchmove=(e)=>{ if(e.touches.length>0){ this.DocOnMouseMoveTitle(e.touches[0]); } }
        document.ontouchend=(e)=>{ this.DocOnMouseUpTitle(e); }
    }
    ...
    this.DocOnMouseMoveTitle=function(e)
    {
        ...
        // 上/左边界检查：保证标题栏始终在可见区域内
        if (top < scrollPos.Top) top = scrollPos.Top;
        if (left < 0) left = 0;
    ...
    this.Show=function(x, y)
    {
        ...
        // 初始位置边界检查：保证小窗在可见区域内
        if (y < scrollPos.Top) y = scrollPos.Top;
        if (x < 0) x = 0;
