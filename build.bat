...
echo [INFO] README.md: merging RELEASE.md download section before render

REM Use PowerShell to merge: find "## 禅中看缠" heading, replace until next "## " heading.
...
powershell -NoProfile -Command "$r='!SYNC_README_FILE!'; $rel='!SYNC_RELEASE_FILE!'; $tmp='!SYNC_TMP!'; $lines=Get-Content $r -Encoding UTF8; $h=-1; for($i=0;$i -lt $lines.Count;$i++){if($lines[$i] -match '^## 禅中看缠'){$h=$i;break}}; if($h -lt 0){exit 5}; $n=-1; for($i=$h+1;$i -lt $lines.Count;$i++){if($lines[$i] -match '^## '){$n=$i;break}}; $f=if($h -gt 0){$lines[0..($h-1)]}else{@()}; $rl=Get-Content $rel -Encoding UTF8; $rem=if($n -ge 0){$lines[$n..($lines.Count-1)]}else{@()}; ($f+$rl+@('')+$rem) -join [Environment]::NewLine | Set-Content $tmp -Encoding UTF8"
if errorlevel 5 (
    echo [WARN] README.md: download section heading not found, skipping RELEASE.md sync
    del "!SYNC_TMP!" >nul 2>&1
    exit /b 0
)
fc /b "!SYNC_TMP!" "!SYNC_README_FILE!" >nul 2>&1
if not errorlevel 1 (
    REM Identical — no need to overwrite, just clean up
    del "!SYNC_TMP!" >nul 2>&1
    exit /b 0
)
echo [INFO] README.md: download section updated from RELEASE.md
REM Content differs — overwrite README.md
...
:need_rebuild
REM Incremental build check via robocopy snapshots (SMB-safe, no PowerShell).
...
if not defined RB_SNAP (
    ...
    set "RB_OUT=%~1"
    if not exist "!RB_OUT!" set "RB_NEED=1"
    if not exist "!RB_SNAP!\" set "RB_NEED=1"
) else if "!RB_NEED!"=="0" (
...
if exist "!SC_SRC!\" goto :nr_check_dir
...
robocopy "!SC_SRC!" "!SC_SNAP!\%~nx2" /E /L !RB_XD! /NFL /NDL /NJH /NJS /NP >nul 2>&1
if !ERRORLEVEL! GTR 0 set "RB_NEED=1"
exit /b 0
...
if not defined OBF_VER echo [WARN] Cannot read crate version from %OBF_CARGO%, using seed 42
...
call :snap_update "apk_%PROFILE%" --exclude "%ANDROID_DIR%\zen_mobile\app\src\main\assets" "%ANDROID_DIR%\web" "%DESKTOP_CORE_WEB%" "%ANDROID_DIR%\zen_mobile\app\src\main" "%DIST_ROOT%\%TARGET_WASM%\%PROFILE%\pkg\%WASM_LIB_NAME%" "%DIST_ROOT%\aarch64-linux-android\%PROFILE%\zen_android_api.aar" "..\src\auth" "..\src\common" "..\src\key" "..\src\lib.rs" "..\src\interfaces\mod.rs" "..\src\interfaces\interface_wasm" "..\src\interfaces\interface_android" "..\src\indicators" "..\src\kline" "..\src\market" "..\src\movement" "..\src\pivot" "..\src\segment" "..\src\stroke" "..\Cargo.toml" "..\Cargo.lock"
