...
(function() {
    ...
    // 读取主程序下发的 serverToken（用于 /license-ping 与 /license-decision 鉴权）
    ...
    var pingTimer = setInterval(function() {
        fetch(withToken('/license-ping'), { method: 'POST', keepalive: true }).catch(function() {});
    }, 500);

    // onbeforeunload：浏览器即将关闭时立即通知后端（sendBeacon 比 keepalive 更可靠）
    window.addEventListener('beforeunload', function() {
        clearInterval(pingTimer);
        navigator.sendBeacon(withToken('/license-ping'), '');
    });
    ...
    agreeBtn.addEventListener('click', function() {
        ...
        .then(function() {
            ...
            if (serverToken) {
                setTimeout(function() {
                    window.location.replace('/?token=' + encodeURIComponent(serverToken));
                }, 500);
            } else {
                setTimeout(function() {
                    window.open('', '_self');
