...
func CORSHandler(h http.HandlerFunc, options APIOptions) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if options.AuthToken != "" {
			...
			if reqToken == "" && options.AuthCookieName != "" {
				// 浏览器导航不带 ?token= 时回退到 HTML 入口种下的 cookie
				if c, err := r.Cookie(options.AuthCookieName); err == nil {
					reqToken = c.Value
				}
			}
...
func MinuteHandler(w http.ResponseWriter, r *http.Request) {
	...
	ss := Sym(s)
	// 与 realtime 同款约束：腾讯 minute/query 美股只认去交易所后缀形式（usAAPL.OQ -> usAAPL），
	// 带后缀返回空数据；A股/港股维持原样
	if IsUS(s) {
	...
	if qq.Code == 0 {
		if d, ok := qq.Data[ss]; ok {
			...
			for _, line := range d.Data.Data {
				...
				// A股行为 4 字段（time price vol amount），美股行只有 3 字段（无成交额），
				// 按 <3 过滤，否则美股全部行被丢弃 → 迷你图空白
				if len(parts) < 3 {
					continue
				}
