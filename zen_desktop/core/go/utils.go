...
func Sym(s string) string {
	...
	// 美股带交易所后缀（aapl.oq.us -> usAAPL.OQ），K线接口要求完整后缀
	if strings.HasSuffix(upper, ".US") {
		return "us" + upper[:len(upper)-3]
	}
...
func SymRealtime(s string) string {
	...
	// 美股行情接口只认去交易所后缀形式（usAAPL.OQ -> usAAPL）
...
// IsUS 美股判定：行情量纲为股（与 A 股"手"不同，量不乘 100）
func IsUS(s string) bool {
	upper := strings.ToUpper(s)
	return strings.HasSuffix(upper, ".US") || strings.HasPrefix(Sym(s), "us")
}
