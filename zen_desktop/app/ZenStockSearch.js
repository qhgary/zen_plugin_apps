(function() {
            ...
            if (self.isVisible) {
                switch (e.key) {
                    ...
                    case 'ArrowUp':
                        ...
                        return;
                    case 'Enter':
                        // 回车等同点击"新增"按钮：有候选时添加最上方（最匹配）的一项，
                        // 无候选不做任何反应（见 tryAddCurrent）
                        e.preventDefault();
                        self.tryAddCurrent();
                        return;
                    case 'Escape':
        ...
        fetch(url, { headers: headers })
            ...
            .then(function(data) {
                if (data && data.success && Array.isArray(data.results)) {
                    self.results = data.results.slice(0, CONFIG.MAX_RESULTS);
    ...
    StockSearch.prototype.selectItem = function(index) {
        ...
        if (addBtn && typeof addBtn.onclick === 'function') {
            // 打上"经智能列表选中"的一次性标记；doAddWatch 只放行带标记的提交
            addBtn.dataset.zenPicked = fullCode;
            addBtn.onclick();
        }
    };

    // "新增"按钮 / 回车共用逻辑：智能搜索有候选时添加列表第一项（服务端按匹配度排序，最上方即最匹配）；
    // 无候选不做任何反应。也可点击/触摸列表中的具体股票改选。
    StockSearch.prototype.tryAddCurrent = function () {
        if (this.results.length > 0) {
            this.selectItem(0);
        }
    };
