...
(function() {
        ...
        for (var attempt = 0; attempt < REQUEST_MAX_RETRIES; attempt++) {
            try {
                ...
                if (attempt < REQUEST_MAX_RETRIES - 1) {
                    console.warn("[ZenLocalService] " + label + " 获取失败 (尝试 " + (attempt+1) + "/" + REQUEST_MAX_RETRIES + "), " + REQUEST_RETRY_MS + "ms后重试:", e);
    ...
    var ZenSvc = {
        ...
        async getLicenseInfo() {
            try {
                if (window.ZenAuthBridge && typeof window.ZenAuthBridge.getStatus === "function") {
                    return JSON.parse(window.ZenAuthBridge.getStatus() || "{}");
                }
                ...
                for (var i = 0; i < arySymbol.length; i++) {
                    ...
                    switch(type) {
                        case 100: case 101:
                            ...
                            break;
                        case 3:
                            p = ZenSvc.minute(sym, arySymbol[i].Date);
                            break;
                        case 200:
