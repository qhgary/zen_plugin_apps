...
addEventListener('message', (e) => {
  ...
  } else if (msg.id === 'get_contact_email') {
    (async () => {
      try {
        await initWasm();
        if (typeof get_contact_email === 'function') {
          const email = get_contact_email();
          postMessage({ id: 'contact_email', email: email });
        } else {
          postMessage({ id: 'contact_email', email: '', error: 'Function not available' });
        }
      } catch (e) {
        postMessage({ id: 'contact_email', email: '', error: String(e) });
      }
    })();
  }
});
